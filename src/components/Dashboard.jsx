...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        <h3 className="text-sm font-bold text-gray-700 mb-3">Tarifario</h3>
        <div className="grid grid-cols-2 gap-3">
          {tarifario.map((t, idx) => (
            <div key={t.tarea} className="flex items-center justify-between gap-2">
              <span className="text-sm text-gray-700">{t.tarea}</span>
              <input
                type="number"
                className="w-32 p-2 bg-gray-50 border rounded-lg text-right"
                value={t.precioBase}
                onChange={(e) => {
